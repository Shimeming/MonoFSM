 ...
 public class AnimationWindowNavbar
{
    ...
    private UnityEditor.Animations.AnimatorController currentController; // 追踪當前的AnimatorController
    ...
    public void OnGUI(Rect navbarRect)
    {
        ...
        float animatorObjectField(float startX)
        {
            ...
            // 更新當前的 Animator
            var selectedAnimator = Selection.activeGameObject?.GetComponentInParent<Animator>();
        ...
        Rect searchField()
        {
            ...
            if (currentFocusedName == "AnimationSearchFilter" && currentControlID != 0)
            {
                ...
                // Debug.Log($"Our TextField has focus with controlID: {currentControlID}");
            }
            
            // 移除過度激進的焦點保護 - 這會干擾正常輸入
            // if (isSearchActive && showDropdown && currentFocusedName != "AnimationSearchFilter")
            // {
            //     EditorGUI.FocusTextInControl("AnimationSearchFilter");
            // }
            ...
            showDropdown = !string.IsNullOrEmpty(searchText) && (currentMatches.Any() || isFocused || isNavigatingDropdown);
            // 處理鍵盤事件
            // KeyNavigate();
        ...
        {
            if (!currentMatches.Any() && string.IsNullOrEmpty(searchText)) return;
            ...
            var actualItemHeight = 18f; // 更緊湊的高度
            ...
            // dropdownRect.DrawOutline(borderColor, 1);
            ...
            if (currentMatches.Any())
            {
                for (var i = 0; i < Math.Min(currentMatches.Count, maxDropdownItems); i++)
                {
                    ...
                    labelStyle.normal.textColor = isSelected ? Color.white : 
                        (isDarkTheme ? new Color(0.85f, 0.85f, 0.85f) : new Color(0.2f, 0.2f, 0.2f));
    ...
    private void DrawCreateNewClipOption(Rect itemRect, string clipName)
    {
        ...
        var labelStyle = new GUIStyle(EditorStyles.label);
        labelStyle.normal.textColor = isSelected ? Color.white : 
            (isDarkTheme ? new Color(0.7f, 0.7f, 0.7f) : new Color(0.4f, 0.4f, 0.4f));
        labelStyle.fontSize = 11;
        labelStyle.padding.left = 8;
    ...
    private void ClearSearch()
    {
        ...
        showDropdown = false;
        isNavigatingDropdown = false;
        isCreateOptionSelected = false; // 重置創建選項選中狀態
        // shouldFocusOnNextRepaint = false;
        Debug.Log("Search cleared");
        // searchText = "";
    }
    ...
    private List<AnimationClip> FindMatchingAnimationClips(string searchTerm)
    {
        ...
        try
        {
            ...
            //         if (clip != null && clip.name.IndexOf(searchTerm, StringComparison.OrdinalIgnoreCase) >= 0)
            //         {
            //             clips.Add(clip);
            //         }
            //     }
    ...
    private void CreateAnimationClipAndState(string stateName)
    {
        try
        {
            ...
            var animatorUtilityType = System.Type.GetType("MonoFSM.AnimatorUtility.AnimatorAssetUtility, MonoFSM.Core");
            if (animatorUtilityType != null)
            {
                var addStateMethod = animatorUtilityType.GetMethod("AddStateAndCreateClipToLayerIndex", 
    ...
    private void CreateStateAndClipManually(UnityEditor.Animations.AnimatorController controller, string stateName)
    {
        // 獲取基礎層的狀態機
        ...
        var clipPath = System.IO.Path.Combine(controllerDir, $"{stateName}.anim");
