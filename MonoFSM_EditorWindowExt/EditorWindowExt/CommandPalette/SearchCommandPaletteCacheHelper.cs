    ...
    public static class SearchCommandPaletteCacheHelper
    {
        ...
        private static string GetDisplayNameFromMenuPath(string menuPath)
        {
            ...
            if (shortcutIndex > 0 && cleanPath.Length > shortcutIndex + 1)
            {
                ...
                if (possibleShortcut.Contains('%') || possibleShortcut.Contains('#') || possibleShortcut.Contains('&'))
                {
        ...
        /// 儲存MenuItem快取到檔案
        /// </summary>
        public static void SaveMenuItemCacheToFile(List<MenuItemEntry> menuItems, string cacheFilePath)
        {
            try
            {
                if (!Directory.Exists(CacheDirectory))
                ...
                {
                    cacheTimestamp = DateTime.Now.Ticks,
                    menuItems = menuItems.Select(item => new MenuItemCacheData(
                        item.menuPath,
                        item.displayName,
                        item.category,
                        item.isValidated,
                ...
                var json = JsonUtility.ToJson(cacheData, true);
                File.WriteAllText(cacheFilePath, json);
                Debug.Log($"[CommandPalette] MenuItem快取已儲存至 {cacheFilePath}，包含 {menuItems.Count} 個項目");
            }
            catch (Exception e)
            {
        ...
        public static List<MenuItemEntry> LoadMenuItemCacheFromFile(string cacheFilePath)
        {
            try
            {
                ...
                var menuItems = cacheData.menuItems
                    .Select(cached => new MenuItemEntry(cached))
                    .ToList();

                stopwatch.Stop();
                Debug.Log($"[CommandPalette] MenuItem快取載入完成，共 {menuItems.Count} 個項目，耗時 {stopwatch.ElapsedMilliseconds}ms");
