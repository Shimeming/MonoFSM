...
namespace MonoFSM.Core.Editor
{
    ...
    public class FieldPathPropertySelector : OdinSelector<string>
    {
        ...
        protected override void BuildSelectionTree(OdinMenuTree tree)
        {
            ...
            // 建立屬性分組
            var propertyGroups =
                new Dictionary<string, List<(string name, Type type, MemberInfo member)>>();

            // 獲取所有可讀的公共屬性
            var properties = _targetType
        ...
        private string GetGroupName(Type type)
        {
            ...
            // MonoFSM 相關型別
            if (type.Name.StartsWith("Var"))
                return "變數";
            if (type.Name.Contains("MonoEntity"))
                return "實體";
            if (type.Name.Contains("Item") || type.Name.Contains("Slot"))
                return "物品系統";
