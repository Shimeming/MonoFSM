...
namespace MonoFSM.Core
{
    ...
    public class SOPathSetting
    {
        ...
        private void PingPath()
        {
            ...
            while (
                !string.IsNullOrEmpty(pathToCheck)
                && pathToCheck != "Assets"
                && !pathToCheck.StartsWith("Packages/")
            )
            {
            ...
            )
            {
                try
                {
                    ...
                    // 創建後再次嘗試定位
                    UnityEngine.Object newAsset = AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(
                        fullPath
                    );
                    if (newAsset != null)
                    {
    ...
    [CreateAssetMenu(
        fileName = "ScriptableObjectPathConfig",
        menuName = "Config/ScriptableObject Path Config"
    )]
    public class SOPathSettingConfig : ScriptableObjectSingleton<SOPathSettingConfig>
    {
        ...
        [FormerlySerializedAs("GlobalFolderRootName")]
        public string _globalFolderRootName = "10_Scriptables";
        ...
        private void RefreshPathSettings()
        {
            foreach (var setting in pathSettings)
            {
                ...
                {
                    Debug.LogWarning(
                        $"型別 {setting.typeName} 的路徑 {setting.targetPath} 無效，已重置為 Assets"
                    );
