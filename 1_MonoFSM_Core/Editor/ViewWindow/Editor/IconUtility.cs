...
public static class EditorIconUtility
{
    ...
    public static Texture2D GetIcon(Type forScriptType)
    {
        ...
        return (Texture2D)EditorGUIUtility
            .ObjectContent(s_scriptLookup.TryGetValue(forScriptType, out var script) ? script : null, forScriptType)
            ?.image;
    }
    ...
    public static Texture2D GetIcon(Object forObject)
    {
        ...
        if (forObject is ScriptableObject || forObject is MonoBehaviour || forObject is GameObject ||
            forObject is MonoScript)
        {
