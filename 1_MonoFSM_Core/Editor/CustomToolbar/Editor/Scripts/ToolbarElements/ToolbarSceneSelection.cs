...
internal class ToolbarSceneSelection : BaseToolbarElement
{
    ...
    public ToolbarSceneSelection() : base(36)
    {
    }
    ...
    private void DrawSceneDropdown()
    {
        selectedSceneIndex = EditorGUILayout.Popup(selectedSceneIndex,
            scenesPopupDisplay.Select(e => e.popupDisplay).ToArray(), ToolbarStyles.popupButtonStyle,
    ...
    void AddScene(string path, string prefix = null, string overrideName = null)
    {
        ...
        if (scenesBuildPath.Contains(path))
            content = new GUIContent("Final: " + name, EditorGUIUtility.Load("BuildSettings.Editor.Small") as Texture,
                "Open scene");
        else
